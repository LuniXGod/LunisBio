...
> **Emoji status note:** the Action downloads your custom-emoji status sticker to
> `assets/status.<ext>` and records `status_type` in `profile.json`:
> - **animated** stickers → `status.tgs` (Lottie); the page lazy-loads
>   [pako](https://github.com/nodeca/pako) + [lottie-web](https://github.com/airbnb/lottie-web)
>   from a CDN **only when a `.tgs` is present**, gunzips it, and plays it inline.
> - **video** stickers → `status.webm` (small muted autoplay loop).
> - **static** stickers → `status.webp` (small `<img>`).
> - If there's no sticker, or any of it fails to load, the page falls back to the
>   unicode `emoji_status`, then to `CONFIG.status`. The animated status needs an
>   internet connection for the CDN libraries; the emoji fallback works offline.
