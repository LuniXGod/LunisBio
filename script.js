...
const CONFIG = {
  ...
  /* Full-screen background. Change ONLY this path — the file EXTENSION decides
     how it is rendered, no HTML edits needed:
         .mp4 / .webm        -> looping muted <video> (with autoplay retries)
  ...
  icons: [
    { image: "assets/icon1.svg", url: "https://telegram.me/FuckLuni",         label: "Projects"  },
    { image: "assets/icon2.svg", url: "https://telegram.me/ModeAlIah",        label: "Direct Message"   },
    { image: "assets/icon3.svg", url: "https://github.com/LuniXGod",   label: "Github" },
  ],

  /* ---- music playlist (add/remove freely) ---- */
  playlist: [
    { title: "Myulee — Prombl", file: "assets/track1.mp3" },
    { title: "Порезы на руках — FORTUNA 812", file: "assets/track3.mp3" },
    { title: "Люблю — БИТЬМРАЗЕЙ", file: "assets/track2.mp3" },
  ],
...
/* ===================== FORMAT-AGNOSTIC BACKGROUND ====================== */
/* Reads CONFIG.background, looks at the extension, and builds the matching
   element into #bg-video-slot. The created node always carries the SAME
   .bg-video class, so sizing / object-fit / z-index / filter are identical no
   matter which format is used. Video-only logic (autoplay retries, the "click
   to start" hint, diagnostics) is attached ONLY when a <video> was created.  */

const VIDEO_TYPES = { mp4: "video/mp4", m4v: "video/mp4", mov: "video/mp4",
                      webm: "video/webm", ogv: "video/ogg", ogg: "video/ogg" };

function extOf(path) {
  // strip ?query / #hash first so "bg.mp4?v=2" still resolves to "mp4"
...
function setBackground() {
  const slot = $("#bg-video-slot");
  const hint = $("#video-hint");
  if (!slot) return;

  const src = CONFIG.background || CONFIG.backgroundVideo || "";
  slot.innerHTML = "";                       // idempotent: safe to call twice
  document.body.classList.remove("video-failed");
  window.__playBgVideo = () => {};           // always defined (no-op for images)

  if (!src) { document.body.classList.add("video-failed"); return; }

  const ext = extOf(src);
  const isVideo = Object.prototype.hasOwnProperty.call(VIDEO_TYPES, ext);

  const onFail = (what) => () => {
    document.body.classList.add("video-failed");
    console.warn(
      `[bg] could not load '${src}' as ${what}. Confirm the file exists` +
      (what === "video"
        ? " and is H.264/AAC MP4 (re-encode with ffmpeg if needed)."
        : " and the extension matches the real file type.")
    );
  };

  /* ---------- IMAGE (.gif / .jpg / .jpeg / .png / .webp / .avif / ...) ------
     No JS needed: gifs animate natively, stills just paint. Deliberately NO
     video listeners here, so nothing can hit a null/none-video reference.  */
  if (!isVideo) {
    const img = document.createElement("img");
    img.className = "bg-video";
    img.alt = "";
    img.decoding = "async";
    img.addEventListener("error", onFail("image"));
    img.src = src;
    slot.appendChild(img);
    hint && hint.classList.remove("show");   // images never need a user gesture
    return;
  }
  ...
  video.autoplay = true;
  video.loop = true;
  video.muted = true;                        // property AND attribute: some
  video.setAttribute("muted", "");           // browsers only honour the attr
  ...
  source.id = "bg-video-src";
  source.src = src;
  if (VIDEO_TYPES[ext]) source.type = VIDEO_TYPES[ext];
  video.appendChild(source);
  slot.appendChild(video);
  video.load();

  video.addEventListener("error", onFail("video"));
  source.addEventListener("error", onFail("video"));
...
function hydrate() {
  ...
  setBackground();   // builds <video>/<img> into #bg-video-slot from CONFIG.background
...
async function loadProfile() {
  try {
    ...
    // animated/static custom-emoji status (tgs/webm/webp) or unicode fallback
    renderStatus(p);
...
/* ---- Telegram custom-emoji status rendering ---- */
const STATUS_BASE = "assets/status";  // the Action writes assets/status.<ext>
...
// Load pako + lottie-web from CDN, but ONLY when a .tgs status actually exists.
let _tgsLibsPromise = null;
...
function ensureTgsLibs() {
  if (_tgsLibsPromise) return _tgsLibsPromise;
  _tgsLibsPromise = Promise.all([
    window.pako   ? Promise.resolve() : loadScriptOnce("https://cdn.jsdelivr.net/npm/pako@2.1.0/dist/pako.min.js"),
    window.lottie ? Promise.resolve() : loadScriptOnce("https://cdn.jsdelivr.net/npm/lottie-web@5.12.2/build/player/lottie.min.js"),
  ]);
...
function initTabTitle() {
  ...
  function tick() {
    const full = texts[ti];
    // NEVER let the tab title be empty/whitespace-only — the browser would flash
    // its fallback (the page URL). Always render at least one visible character.
    document.title = full.slice(0, Math.max(1, ci));
    ...
    if (!deleting) {
      ...
      if (ci > full.length) {
        ...
        if (cfg.deleteBetween === false) {
          // no deleting: hold, then jump straight into the next word at 1 char
          ti = (ti + 1) % texts.length; ci = 1;
      ...
      if (ci <= 1) {
        // stop at 1 char (never 0), then hand that single char off to the next
        // phrase and start typing it — so the title never goes blank.
        ci = 1; deleting = false;
...
function initGate(player) {
  ...
  function enter() {
    ...
    // A video background may have been blocked from autoplay until this very
    // interaction; for image/gif backgrounds this is a harmless no-op.
    window.__playBgVideo?.();
